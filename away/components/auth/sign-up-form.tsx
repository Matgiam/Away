	...
	const handleGoogleSignUp = async () => {
		...
		try {
			const { error } = await supabase.auth.signInWithOAuth({
				provider: "google",
				options: {
					redirectTo: `${getSiteURL()}/auth/callback?next=/protected`,
				},
			});
			if (error) throw error;
		} catch (error: unknown) {
		...
		<div className={cn("w-full max-w-md", className)} {...props}>
			<div className="rounded-2xl border border-white/10 bg-[#0a0118]/80 backdrop-blur-xl shadow-2xl px-8 py-10">
				<div className="mb-8 text-center">
					<h1 className="text-4xl italic font-light text-white mb-2">Sign up</h1>
					<p className="text-sm text-white/40 italic">Create a new account to start playing.</p>
				</div>
